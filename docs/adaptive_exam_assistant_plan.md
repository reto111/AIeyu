...
- 新增 `knowledge_sources` / `knowledge_chunks`。
- 新增 `data/knowledge_sources/tem8/`。
- 已导入一份人工整理的语法出题依据草稿，共 14 个知识块。
- 已提供 Markdown 导入脚本和关键词/标签检索脚本。
- 该草稿不是官方考试大纲；正式大纲或可靠资料应后续单独导入并如实标注来源。
...
- 本地默认用户。
- 题型掌握度和知识点掌握度计算。
- 3 天、7 天、10 天、更早四档时间权重。
- 最近连续错误降级规则。
- 题目曝光记录 `question_exposures`。
- 用户画像接口 `GET /api/profile`。
- 随机组卷已参考曝光记录做基础避重，默认不包含阅读题。
- 前端已提供 30 题入门诊断，诊断模式包含四类题型并尽量均衡抽题。
- 前端已展示四类题型掌握度，并支持选择“只练此类”。
- 已建立 AI 出题前的本地出题依据库和轻量检索脚本。
