...
- 第一版采用自评模式：`不认识`、`模糊`、`认识`、`已掌握`。
- 复习间隔沿用既定规则：1 天、2 天、4 天、10 天。
...
- 右上角菜单支持输入姓名新增学生名单。
- 单词打卡新增进度条、上一词、下一词、稍后再看和完成总结。
- 已处理过的单词卡片会显示“已记录”状态，避免在同一轮打卡中重复提交。

2026-08-22 已完成服务器端学生账号隔离：

- 学生端账号入口改为注册/登录/退出，不再展示全体学生名单。
- 注册使用“姓名 + 访问码”轻量方案，适合第一版课堂/服务器试用。
- 后端新增 `user_auth` 和 `user_sessions`，访问码使用 PBKDF2 哈希存储，浏览器通过 HttpOnly Cookie 保存登录状态。
