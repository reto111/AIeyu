...
## Product scope guardrail（用户于 2026-09-02 再次确认）

- AIeyu 的核心目标始终是“小语种考试个人 AI 自适应备考助手”。所有新增工作必须直接服务于：考试诊断、题库训练、自动批改、错题解析、能力画像、薄弱专项、错题复习或考试词汇复习。
- 不为了展示 AI 能力或追求功能数量而增加旁支模块，不把产品复杂化。能在现有页面和现有学习闭环内完成的能力，不新增独立系统、复杂工作流或多余配置。
- 当前优先级是把专四/专八现有核心闭环稳定到可供学生试用，再根据真实反馈决定后续功能；周报、社交、教师端、复杂运营后台等均不进入当前 MVP。
- 每个候选功能在开发前检查三点：是否提高备考效果、是否减少学生操作成本、是否属于原定产品路线。至少一项不满足时默认不做。

### Latest checkpoint: 题量适配的专项方向重构（2026-09-03）

- 依据现有 744 道正式题的真实分布，将学生侧过细且样本不足的知识点合并为稳定训练池。内部分类器仍记录原有细标签用于题库审核和错题解释，但能力画像、学习中心和一键专项只使用粗粒度方向。
- 语法统一为三个方向：`grammar.forms` 词形与动词系统、`grammar.collocation` 词义/前置词与搭配、`grammar.sentence` 句法/连接与表达。文学统一为 `literature.knowledge`，国情统一为 `culture.knowledge`，阅读统一为 `reading.comprehension` 且学生侧仍只展示阅读题型掌握度。
- 专四训练池为语法 131/100/59、国情 35、阅读 119；专八为语法 30/51/21、文学 42、国情 36、阅读 120。每个方向均不少于 20 题，专八“词形与动词系统”已验证可直接生成 10 道精确专项，无需同题型补题。
- 重标时保留全部历史作答、账号、错题和单词记录；已有每日计划中的旧知识点代码同步映射到新方向，旧激活推荐作废并在下次画像计算时重建。写入前备份：`data/processed/backups/russian_ai_tutor_before_fine_knowledge_tags_20260903_125628.sqlite`。
- 本地数据库完整性为 `ok`，正式题量保持 `TEM4_RU=444`、`TEM8_RU=300`；自动化回归扩充为 14 项并增加“所有学生训练方向至少 20 题”的防回退检查。本地验证地址为 `http://127.0.0.1:8797/`。

### Latest checkpoint: MVP 发布验收与服务器健康检查（2026-09-02）

- 新增 `/api/health`：检查 SQLite 完整性、必要数据表、专四/专八正式题量与词量、前端静态文件、DeepSeek 配置和 `pymorphy3` 词形还原依赖。正常返回 HTTP 200，关键条件缺失返回 HTTP 503；接口不泄露 API Key、密码或连接字符串。
- 新增 `scripts/check_mvp_readiness.py` 作为发布门禁。它在上述健康条件之外继续执行正式练习题高/中风险审计、正式词库基础质量审计、专四/专八状态接口、首页访问以及核心自动化测试；任一项失败即以非零状态退出。
- 对真实本地数据库执行完整验收，共 17 项全部通过：`TEM4_RU=444` 道正式题、3506 个正式词；`TEM8_RU=300` 道正式题、3513 个正式词；题库高/中风险项为 0，词库基础异常为 0，13 项回归测试通过。
- 首次验收实际拦截两条历史词义污染：`набор` 的俄文词头被附加进中文释义，`претензия` 存在同类问题。已加入可重复执行的确定性修正规则并修正数据库，而不是只修改当前显示结果。
- 修正前数据库备份：`data/processed/backups/russian_ai_tutor_before_vocab_quality_fixes_20260902_221517.sqlite`。最新验收报告：`data/processed/health/mvp_readiness_latest.json`。本地完整验收地址：`http://127.0.0.1:8796/`。

### Latest checkpoint: PC / 手机内部划词翻译 1.0（2026-09-01）

- AIeyu 页面内的阅读原文、题干、选项和错题解析支持俄语划词翻译。划词后只在选区旁显示小型“译”按钮，点击后才展开解析框；宽屏优先放左右，手机窄屏优先放上方并按视口自动换位，不固定覆盖底部答题按钮。一次最多选择 6 个词，输入控件和导航不触发。
...
- 推荐专项继续使用用户画像既定的 3/7/10 天时间权重和语法词汇优先权重。每日任务保存知识点、对应题型和推荐原因；首页直接展示原因、目标与真实完成进度。
- 答题任务只统计当天对应题型的作答；错题任务按生成时待巩固基线与当前数量的差值计算；单词任务读取当天真实打卡记录。所有完成状态均不可手工伪造。
- 点击推荐任务会携带 `daily_task_id`，服务器再次验证任务属于当前登录账号、当前考试与当天日期，再按锁定知识点组卷。错题任务可从首页直接生成待巩固重练卷。
- 新增 `daily_study_plans`、`daily_study_tasks` 及对应索引，旧服务器数据库在启动时自动迁移。自动化测试增至 11 项，新增同日计划稳定、无关题型不计进度和跨账号任务拒绝测试。
- 迁移前数据库备份：`data/processed/backups/russian_ai_tutor_before_daily_plan_20260901_195607.sqlite`。迁移后 `PRAGMA integrity_check = ok`，题量保持 `TEM4_RU=444`、`TEM8_RU=300`；真实页面与 390px 手机端无横向溢出，浏览器控制台无错误，测试地址为 `http://127.0.0.1:8794/`。

### Latest checkpoint: 错题本 2.0（2026-09-01）

- 错题本升级为可执行复习闭环：按专四/专八隔离，展示累计错题、待巩固、反复出错和收藏统计，并支持状态、题型、知识点及关键词筛选。
- 每道错题可展开查看完整选项、最近答案、正确答案、阅读原文和作答历史；阅读细分标签不在学生侧展示，只显示整体“阅读理解”。
- 新增学生私有收藏与复盘笔记表 `wrongbook_preferences`。接口始终从登录会话读取用户，写入前再次确认题目属于该账号的真实错题。
- 新增 `wrongbook_review` 组卷模式：支持重练全部待巩固题或勾选题目重练；阅读错题自动扩展为完整文章题组。重练提交继续复用现有批改、AI逐题解析和画像更新，答对后错题状态自动变为“已订正”。
- 新增自动化覆盖收藏/笔记、账号隔离、非法跨账号重练和错题重练组卷。现有专项训练、整篇阅读、随机卷、诊断和题库质量门禁合计10项测试全部通过。
- 真实页面已验证专四7道错题的统计、筛选、选项展开和勾选重练；桌面与390px手机端无横向溢出，浏览器控制台无错误。本地测试地址为 `http://127.0.0.1:8793/`。
- 迁移后数据库备份：`data/processed/backups/russian_ai_tutor_wrongbook_v2_20260901_165702.sqlite`；题量保持 `TEM4_RU=444`、`TEM8_RU=300`，`PRAGMA integrity_check = ok`。

### Latest checkpoint: 阅读画像合并、文学标签重构与专八文学题修复（2026-09-01）

- 学生侧不再展示或推荐 `reading.detail`、`reading.vocabulary_context` 等机械阅读子标签。阅读仍保留题型级掌握度、整篇文章组卷和“只练阅读”能力；内部细标签可继续用于题库质检，但不进入细知识画像和下一步弱项排序。
- 专八文学知识点由按时代切片的“19世纪/白银时代/苏联文学”重构为四个学习任务：`literature.author_work` 作家与作品、`literature.work_content` 人物名句与情节、`literature.history_movements` 文学史与流派、`literature.genre_terms` 体裁与文学术语。42道正式文学题分布为25/9/4/4。
- 修复2017专八第34、35、37题以及2018专八第34至37题。2017第34题原始文本完整，旧双栏切分器误将 `B) И. С. Тургенева` 拆成 `B=И.`、`C=Тургенева` 并吞掉 `C) А. П. Чехова`；现已恢复四个完整选项。
- 随后按新增规则回扫全库并对照原 PDF 逐页复核，继续恢复2017专八第19、40、41、42、48、49、61、63、64题，2018专八第20、32、41、42、48、53、57题，2024专八第20题，以及2017专四第58、60题。修复覆盖姓名被拆断、拉丁/西里尔字母混淆、页码水印吸入、题干断词和阅读选项残缺；所有修改均写入可重复执行脚本，不依赖单次手工改库。
- `scripts/audit_question_bank_quality.py` 新增“文学/国情选项只有单个作者首字母”高风险规则，并检查俄语选项中的连续拉丁 OCR 噪声；`USB` 等合法缩写加入白名单。最终全部已批准练习题高/中风险项为0，剩余14条中风险提示全部属于已隔离的2024 OCR阅读参考资料，不进入练习池。
- `scripts/apply_question_quality_fixes.py` 的题目定位已限定 `TEM8_RU + past_exam_original`，且旧 unresolved 列表不得覆盖后续人工批准。执行时曾发现旧脚本误降级13道后来已人工通过的题，已从写入前备份仅恢复审核状态；最终题数保持 `TEM4_RU=444`、`TEM8_RU=300`。
- 最新写入前备份：`data/processed/backups/russian_ai_tutor_before_question_quality_fixes_20260901_162550.sqlite`；最新重标前备份：`data/processed/backups/russian_ai_tutor_before_fine_knowledge_tags_20260901_162551.sqlite`。最终数据库 `PRAGMA integrity_check = ok`。

### Latest checkpoint: 个人学习中心与知识点自由专项（2026-08-31）

- 学生端新增独立“学习”页，作为登录后的个人学习中心；专四、专八共用考试选择但数据严格按当前登录账号与考试隔离。
- 今日安排固定为三个学生动作：完成推荐专项或入门诊断、处理到期单词或新词打卡、订正待巩固错题。完成度直接读取作答、单词复习和错题暴露数据，不新增人工维护状态。
- 新增 `/api/study-center`：返回今日任务、近 7 天与 30 天答题/正确率/练习次数、近 7 天逐日趋势、题型掌握度和全部可练细粒度知识点。
- 学习中心的细知识点优先展示语法词汇、文学和国情；阅读按2026-09-01新标准只保留题型级掌握度，不再显示机械子标签。每项显示最近窗口掌握分、状态、作答量和可用题量。
- `/api/quiz` 新增 `knowledge_point` 模式。学生可点击任意已入库知识点直接生成 10 题专项；服务器验证知识点属于当前考试，精确题量不足时只在同题型内补足，阅读仍按完整文章题组返回。
- 新增自动化覆盖：手选知识点组卷和学习中心账号隔离。现有专项推荐、同题型回退、整篇阅读、随机卷和入门诊断回归测试共 7 项全部通过。
- 真实页面验证：专四账号点击“前置词搭配”成功生成 10 道专项题；桌面与 390px 移动端无横向溢出，浏览器控制台无错误。本地测试地址为 `http://127.0.0.1:8791/`。
...
- The batch covered exactly 100 pending coordinates from `p333/b14` through `p341/b14`. Ninety-three entries were approved and seven continuation, boundary, or unusable OCR rows were rejected. Clear recoveries included `еоведесство→человечество`, `чемпибдн→чемпион`, `чёрпать→черпать`, `чёстный→честный`, `четвёрг→четверг`, `чтёние→чтение`, `узко→чужой`, `чулбк→чулок`, `ткестнатикать→шестнадцать`, `шёя→шея`, `шинёль→шинель`, `шитьь→шить`, `тиляпа→шляпа`, `титн→щи`, and `ехозяйства` was correctly rejected as an `экономика` continuation.
- True `ё` was preserved in verified words such as `чёрный`, `чёрт`, `чертёж`, `четвёрка`, `четвёртый`, `чёткий`, `шахтёр`, `шёлк`, and `щётка`; false OCR `ё` was corrected only after checking the meaning. One importer rule collision on `число` was corrected from a Chinese meaning beginning with the part-of-speech marker `数` to `数字；数值；数量；日期` before import.
- Review sheet status after this batch: `approved` = 3464, `rejected` = 405, `needs_review` = 1, `pending` = 54. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3457 unique words; `TEM8_RU/TEM8` = 3513. Dry-run and real import both reported `inserted` = 93, `updated` = 3371, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch38_20260829_125907.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch38_20260829_125907.sqlite`.
- Correction backups: `data/processed/backups/tem4_words_review_simple_before_batch38_import_validation_fix_20260829_130042.csv` and `data/processed/backups/russian_ai_tutor_before_batch38_import_validation_fix_20260829_130042.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Final post-import audit found zero Cyrillic characters in approved Chinese meanings, zero importer-invalid approved rows, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 37 (2026-08-29)

- The thirty-seventh pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p325/b11` through `p333/b13`. Eighty-six entries were approved and 14 continuation, boundary, or unrecoverable OCR rows were rejected. A coordinate-offset error was caught before database import: the regenerated preview included `p325/b16` and `p326/b1`, so the CSV was restored from backup and the first page boundary was remapped before reapplying decisions.
- Important recoveries included `фотографировать`, `фотография`, `форма`, `формироваться`, `характеристика`, `химический`, `ход`, `хозяин`, `хозяйство`, `хранить`, `худой`, `цвет`, `цифра`, `часовой`, `частный`, `чей`, `человек`, and `человеческий`. True `ё` was retained in verified forms such as `церковь`/`часы` where appropriate; false OCR and accent noise were normalized only after checking the word and meaning together.
- Review sheet status after this batch: `approved` = 3371, `rejected` = 398, `needs_review` = 1, `pending` = 154. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3364 unique words; `TEM8_RU/TEM8` = 3513. Dry-run reported `inserted` = 86 and `updated` = 3285; real import reported `inserted` = 85, `updated` = 3286, `invalid_approved` = 0, and `skipped` = 0 because one duplicate word/part-of-speech pair became an update during import.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch37_20260829_124727.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch37_20260829_124727.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero importer-invalid approved rows, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 36 (2026-08-29)

- The thirty-sixth pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p317/b3` through `p325/b10`. Ninety-six entries were approved and four continuation or boundary fragments were rejected. The `умственный` continuation, `условие`/`устанавливать`, `уступать` continuation, and `удовольствие`/`уезд` boundaries were checked explicitly.
- Important recoveries included `уменьшать`, `уметь`, `уничтожать`, `управление`, `упускать`, `успех`, `устанавливать`, `уступать`, `утверждать`, `ухаживать`, `учитывать`, `учёба`, `учебник`, `учреждение`, `факультет`, `филолог`, and `фонарь`. False OCR `ё` was corrected in forms such as `умёть`, `управлёние`, `увёренность`, `уёзд`, `учёбник`, `учёбный`, `учёние`, and `учреждёние`; true `ё` was preserved in `тяжёлый`-style verified forms.
- Review sheet status after this batch: `approved` = 3285, `rejected` = 384, `needs_review` = 1, `pending` = 254. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3279 unique words; `TEM8_RU/TEM8` = 3513. Dry-run reported `inserted` = 96 and `updated` = 3189; real import reported `inserted` = 95, `updated` = 3190, `invalid_approved` = 0, and `skipped` = 0 because one duplicate word/part-of-speech pair became an update during import.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch36_20260829_124051.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch36_20260829_124051.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 35 (2026-08-29)

- The thirty-fifth pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p308/b13` through `p317/b2`. Ninety entries were approved and 10 continuation, merged-boundary, or unusable fragments were rejected. The `трудящийся`/`трус`, `трубка` continuation, `тянуть`/`тянуться`, `увлекать`/`увлекаться`, `удаваться`/`Опыт`, `удивляться` government continuation, `удовольствие`/`уезд`, and `укладывать` object continuation boundaries were checked explicitly.
- Important recoveries included `тройка`, `труд`, `трудиться`, `трудящийся`, `тщательный`, `тушить`, `тяжело`, `тяжёлый`, `тянуть`, `тянуться`, `убеждать`, `уважение`, `уверенность`, `увлекаться`, `удовольствие`, `удивляться`, `узел`, `указать`-related forms, `улыбаться`, `улыбка`, and `умение`. False OCR `ё` was corrected in forms such as `тёма`-style source noise, `сумёть`-style prior boundaries, `уважёние`, and `умёние`; true `ё` was retained in `тяжёлый`.
- Review sheet status after this batch: `approved` = 3189, `rejected` = 380, `needs_review` = 1, `pending` = 354. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3184 unique words; `TEM8_RU/TEM8` = 3513. Dry-run and real import both reported `inserted` = 90, `updated` = 3099, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch35_20260829_123525.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch35_20260829_123540.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 34 (2026-08-29)

- The thirty-fourth pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p300/b4` through `p308/b12`. Ninety entries were approved and 10 continuation, merged-boundary, or unusable fragments were rejected. The `тень`/`теория`, `терять`/continuation, `тот` continuation, and `точно` continuation boundaries were checked explicitly.
- Important recoveries included `тема`, `темнеть`, `тёмный`, `терапевт`, `терпение`, `терпеть`, `тесный`, `техника`, `течение`, `типография`, `товарищ`, `тогда`, `толк`, `ехать`, `тон`, `тонна`, `торчать`, `тотчас`, `точно`, `трагедия`, `требование`, `третий`, and `тело`. False OCR `ё` was corrected in forms such as `тёма`, `темнёть`, `тёсный`, `тёхника`, `течёние`, `трагёдия`, `трётий`, and `тёло`; true `ё` was preserved in `тёмный`, `тёплый`, `тётя`, `твёрдый`, and `счёт`, while `торжественный` was kept without `ё`.
- Review sheet status after this batch: `approved` = 3099, `rejected` = 370, `needs_review` = 1, `pending` = 454. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3094 unique words; `TEM8_RU/TEM8` = 3513. Dry-run reported `inserted` = 90 and `updated` = 3009; the real import reported `inserted` = 89, `updated` = 3010, `invalid_approved` = 0, and `skipped` = 0 because one duplicate word/part-of-speech pair became an update during the same import.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch34_20260829_122952.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch34_20260829_123016.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 33 (2026-08-29)

- The thirty-third pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p290/b14` through `p300/b3`. Ninety-one entries were approved and nine continuation, merged-boundary, or unusable fragments were rejected. The `стоять`/`погода`, `строй` duplicate blocks, `стыдно`/`суметь`, `сухой` continuation, `сын` continuation, and `тайный` continuation boundaries were checked explicitly.
- Important recoveries included `странный`, `сторож`, `стоять`, `страдать`, `страница`, `стрелять`, `стремление`, `строитель`, `строительный`, `строй`, `стройный`, `стыдно`, `судьба`, `суметь`, `сушить`, `существенный`, `существо`, `сцена`, `стихотворение`, `столетие`, and `столовая`. False OCR `ё` was corrected in forms such as `смёлость`-style adjacent source noise, `стоть`, `стройтель`, `сумёть`, `суроври`, `сущёственный`, `сцёна`, `стихотвореёние`, `столётие`, and `тёло`; true `ё` was retained in `твёрдый` and `счёт`.
- Review sheet status after this batch: `approved` = 3009, `rejected` = 360, `needs_review` = 1, `pending` = 554. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 3005 unique words; `TEM8_RU/TEM8` = 3513. The first import audit found one `meaning_contains_pos_marker` on `считать` because its meaning began with the Chinese character `数`; it was changed to `计数`, re-imported, and the final import reported `inserted` = 1, `updated` = 3008, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch33_20260829_122334.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch33_20260829_122349.sqlite`.
- Correction backups: `data/processed/backups/tem4_words_review_simple_before_batch33_import_validation_fix_20260829_122507.csv` and `data/processed/backups/russian_ai_tutor_before_batch33_import_validation_fix_20260829_122507.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Final post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 32 (2026-08-29)

- The thirty-second pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked by meaning and word shape; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, and true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p281/b12` through `p290/b13`. Eighty-six entries were approved and 14 continuation, merged-boundary, or unusable fragments were rejected. The `сотрудничать`/continuation, `сочетать`/`сочинение`, `спускаться`/`средство`, `сражаться`/`сразу`, `ставить`/`вопросы`, and `становиться`/`старший` boundaries were checked explicitly.
- Important recoveries included `состояться`, `сотрудничать`, `социальный`, `сочетание`, `сочетать`, `сочинение`, `специалист`, `список`, `способность`, `способствовать`, `средство`, `срывать`, `ставить`, `стадион`, `сталь`, `становиться`, `соседний`, `ствол`, `степень`, `стихотворение`, `столетие`, and `столовая`. False OCR `ё` was corrected in forms such as `сочинёние`, `срёдство`, `сосёдний`, and `столётие`; true `ё` was preserved in `солёный`.
- Review sheet status after this batch: `approved` = 2918, `rejected` = 351, `needs_review` = 1, `pending` = 654. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2914 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 85, `updated` = 2833, `invalid_approved` = 0, and `skipped` = 0; the one-row difference between approvals and inserts was an existing-word deduplication/update.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch32_20260829_121744.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch32_20260829_121800.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.
...
- The batch covered exactly 100 pending coordinates from `p264/b3` through `p272/b10`. Eighty-nine entries were approved and 11 page fragments, continuation fragments, merged boundaries, or unusable headwords were rejected. The `сильный`/`символ`, `слово`/`сложный`, and `слышно`/`громче` boundaries were checked explicitly.
- Important recoveries included `сельский`, `сельскохозяйственный`, `семестр`, `семьдесят`, `сердечный`, `сердитый`, `сердце`, `серия`, `сессия`, `сеять`, `сигарета`, `сидеть`, `сила`, `система`, `скорее`, `скучать`, `слесарь`, `слово`, `случайный`, `слышаться`, `слышно`, and `слышный`. True `ё` was retained only where justified, such as `серьёзный`; false OCR `ё` was corrected in forms such as `семёстр`, `сёрдце`, `сигарёта`, and `сёмьдесят`.
- Review sheet status after this batch: `approved` = 2744, `rejected` = 325, `needs_review` = 1, `pending` = 854. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2741 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 89, `updated` = 2655, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch30_20260829_120606.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch30_20260829_120625.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 29 (2026-08-29)

- The twenty-ninth pending batch was reviewed directly with the assistant model. Russian headwords and Chinese meanings were cross-checked; false OCR `ё` was corrected to `е` only where standard spelling requires `е`, while true `ё` was retained. Examples, mixed-language OCR, page fragments, continuations, and inflection-only rows were removed; approved rows keep concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p255/b2` through `p264/b2`. Ninety-two entries were approved and eight page fragments, continuation fragments, or inflection fragments were rejected. The fixed expression `быть на седьмом небе` was restored as a meaningful vocabulary item rather than retaining its polluted OCR headword. The only unresolved coordinate remains `p154/b2`.
- Important recoveries included `рождение`, `ронять`, `роса`, `руководить`, `самодеятельность`, `санаторий`, `сантиметр`, `сведение`, `свежий`, `сверху`, `светлый`, `свидетельство`, `свистеть`, `сдерживать`, `север`, `секрет`, and `село`. The `самолёт`, `ружьё`, and `свёртывать` spellings preserve real `ё`; `рождёние`, `свёжий`, `свёрху`, `свидётельство`, `свистёть`, `сёвер`, and `секрёт` were corrected to standard `е` forms.
- Review sheet status after this batch: `approved` = 2655, `rejected` = 314, `needs_review` = 1, `pending` = 954. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2652 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 92, `updated` = 2563, `invalid_approved` = 0, and `skipped` = 0. The first import attempt stopped before any write because it used the TEM8 default system; the corrected TEM4 dry-run and real import both completed successfully.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch29_20260829_115734.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch29_20260829_115803.sqlite`.
- Pre-apply coverage audit matched all 100 batch coordinates exactly. Post-import audit found zero Cyrillic characters in approved Chinese meanings, zero invalid TEM4 word heads, and only the pre-agreed `p154/b2` unresolved coordinate.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 28 (2026-08-29)

- The twenty-eighth pending batch was reviewed directly with the assistant model. OCR symbols, mixed-language fragments, examples, inflection fragments, and page continuations were removed; approved rows retain concise Chinese core meanings.
- The batch covered exactly 100 pending coordinates from `p245/b6` through `p255/b1`. Ninety-three entries were approved and seven page fragments, inflection fragments, merged blocks, or incomplete page blocks were rejected. The only unresolved coordinate remains `p154/b2`.
- Important recoveries included `разочаровывать`, `разрешение`, `разуметься`, `район`, `ракета`, `рассказ`, `растение`, `растериваться`, `революционер`, `редкий`, `режиссёр`, `резать`, `резерв`, `резкий`, `ремонт`, `репетиция`, `рецепт`, `решение`, `рисковать`, `робкий`, `родить`, and `родиться`. False OCR `ё` was corrected to `е` only where the standard word requires `е`; true `ё` was retained.
- Review sheet status after this batch: `approved` = 2563, `rejected` = 306, `needs_review` = 1, `pending` = 1054. The unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2560 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 93, `updated` = 2470, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/backups/tem4_words_review_simple_before_pending_batch28_20260829_115153.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch28_20260829_115153.sqlite`.
...
- The batch covered exactly 100 pending coordinates from `p210/b9` through `p219/b4`. Continuation, duplicate, and fixed-phrase fragments were rejected at `p211/b2`, `p212/b1`, `p213/b1`, `p213/b6`, `p216/b1`, `p217/b1`, `p217/b12`, and `p218/b2`.
- Important OCR recoveries included `помещик`, `понедельник`, `поражать`, `поражение`, `по-прежнему`, `поддерживать`, `подъезд`, `поехать`, `пожелание`, `поить`, `постель`, `поток`, `поэзия`, `поэтому`, and `полотенце`. False `ё` was corrected to `е` in `помещик`, `понедельник`, `подъезд`, `поехать`, `постель`, and `поражение`; the `получать`/`письмо`, `питание`/`питаться`, and `поступать`/`поступок` boundaries were checked explicitly.
- Review sheet status after this batch: `approved` = 2204, `rejected` = 265, `needs_review` = 1, `pending` = 1454. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2201 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 92, `updated` = 2112, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch24_20260828_223701.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch24_20260828_223719.sqlite`.
...
- The batch covered exactly 100 pending coordinates from `p192/b12` through `p201/b7`. Continuation, duplicate, fixed-phrase, and merged fragments were rejected at `p194/b1`, `p195/b2`, `p195/b4`, `p195/b7`, `p195/b13`, `p196/b1`, `p197/b1`, `p198/b1`, `p199/b1`, and `p200/b1`.
- Important OCR recoveries included `перед`, `передний`, `перемена`, `перо`, `песня`, `петух`, `печенье`, `пища`, `платок`, `плащ`, `площадь`, `плыть`, `по`, `победа`, `поведение`, and `поверхность`. False `ё` was corrected to `е` in `передний`, `песня`, `печенье`, `победа`, and `поверхность`; the `питание`/`питаться` and `по`/`поворачивать` boundaries were checked explicitly.
- Review sheet status after this batch: `approved` = 2024, `rejected` = 245, `needs_review` = 1, `pending` = 1654. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 2021 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 90, `updated` = 1934, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch22_20260828_222314.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch22_20260828_222329.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, zero Cyrillic characters in approved Chinese meanings, and verified the corrected headwords in the formal database.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 21 (2026-08-28)

- The twenty-first pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p184/b2` through `p192/b11`. Continuation, duplicate, fixed-phrase, and merged fragments were rejected at `p184/b3`, `p185/b1`, `p185/b6`, `p187/b1`, `p188/b1`, `p189/b1`, `p190/b1`, `p191/b1`, and `p191/b15`.
- Important OCR recoveries included `открытие`, `отрезать`, `от;` as the corrected `от` entry, `офицер`, `охота`, `оценивать`, `оценка`, `ощупывать`, `павильон`, `палец`, `пароход`, `патриот`, `певец`, and `пенсия`. The `открытый`/`открытка`, `павильон`/`парикмахерская`, and `пасти`/continuation boundaries were checked explicitly; false `ё` was corrected to `е` in `офицер`, `певец`, and `пенсия`.
- Review sheet status after this batch: `approved` = 1934, `rejected` = 235, `needs_review` = 1, `pending` = 1754. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1931 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 91, `updated` = 1843, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch21_20260828_221619.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch21_20260828_221635.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, zero Cyrillic characters in approved Chinese meanings, and verified the corrected headwords in the formal database.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 20 (2026-08-28)

- The twentieth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p175/b4` through `p184/b1`. Continuation, fixed-phrase, and garbled fragments were rejected at `p175/b9`, `p176/b1`, `p176/b13`, `p177/b10`, `p178/b11`, `p181/b3`, `p181/b7`, `p181/b8`, `p183/b5`, `p183/b14`, and `p184/b1`.
- Important OCR recoveries included `около`, `олимпиада`, `опыт`, `орден`, `орех`, `оркестр`, `освобождать`, `осенний`, `остров`, `от`, `отвечать`, and `отечественный`. The cross-page `опираться`, `освобождать`, and `отзываться` entries were kept separate from their continuation rows, and only concise core Chinese meanings were retained.
- Review sheet status after this batch: `approved` = 1843, `rejected` = 226, `needs_review` = 1, `pending` = 1854. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1840 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 89, `updated` = 1843, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch20_20260828_220511.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch20_20260828_220652.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, zero Cyrillic characters in approved Chinese meanings, and verified the corrected headwords and meanings in the formal database.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 19 (2026-08-28)

- The nineteenth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p166/b4` through `p175/b3`. Continuation, page-footer, and merged fragments were rejected at `p167/b1`, `p167/b6`, `p167/b8`, `p167/b11`, `p168/b2`, `p169/b1`, `p170/b1`, `p170/b4`, `p171/b1`, `p172/b5`, `p173/b1`, `p173/b7`, `p174/b1`, `p174/b8`, and `p175/b2`.
- Important recoveries included `обмениваться`, `обнаруживать`, `обобщать`, `оборона`, `образ`, `обращение`, `обстоятельство`, `обувь`, `обходиться`, `общественный`, `объём`, `объяснение`, `обыкновенный`, `овца`, `огонь`, `огород`, `одерживать`, `одиннадцать`, `невеста`, and `небрежно`. The incomplete but certain `облегчать`, `обманывать`, and `обеспечивать` entries were supplemented only from their unambiguous headwords and adjacent definitions.
- Review sheet status after this batch: `approved` = 1754, `rejected` = 215, `needs_review` = 1, `pending` = 1954. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1751 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 85, `updated` = 1669, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch19_20260828_215401.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch19_20260828_215434.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, and zero Cyrillic characters in approved Chinese meanings.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 18 (2026-08-28)

- The eighteenth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p158/b6` through `p166/b3`. Continuation, page-footer, and merged fragments were rejected at `p158/b7`, `p159/b1`, `p159/b7`, `p160/b1`, `p160/b12`, `p161/b1`, `p161/b10`, `p164/b1`, `p164/b6`, `p165/b10`, and `p166/b1`.
- Important recoveries included `невеста`, `неделя`, `нежный`, `незаметный`, `незачем`, `неизвестный`, `некоторый`, `немедленный`, `немец`, `немецкий`, `неожиданный`, `неподвижный`, `непременный`, `непрерывный`, `неужели`, `нигде`, `нижний`, `ничей`, `новоселье`, `нос`, `небо`, `обед`, and `обеспечивать`. The `находить`/`находиться` and `обеспечивать`/`облегчать` boundaries were kept separate in the preceding batch.
- Review sheet status after this batch: `approved` = 1669, `rejected` = 200, `needs_review` = 1, `pending` = 2054. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1666 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 89, `updated` = 1580, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch18_20260828_214831.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch18_20260828_214858.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, and zero Cyrillic characters in approved Chinese meanings.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 17 (2026-08-28)

- The seventeenth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p148/b4` through `p158/b5`. Continuation or page-header fragments were rejected at `p149/b1`, `p151/b5`, `p152/b1`, `p152/b3`, `p152/b16`, `p153/b9`, `p153/b13`, `p155/b1`, `p156/b1`, `p157/b2`, `p157/b4`, `p157/b9`, and `p157/b11`.
- Important recoveries included `набирать`, `надежда`, `надеяться`, `наказывать`, `наклеивать`, `накрывать`, `наливать`, `намерение`, `намечать`, `напоминать`, `направление`, `народ`, `население`, `настроение`, `научно-исследовательский`, `научно-технический`, `находить`, `небо`, `небрежный`, and `небрежно`. The `находить`/`находиться` boundary was explicitly checked so their meanings were not swapped.
- Review sheet status after this batch: `approved` = 1580, `rejected` = 189, `needs_review` = 1, `pending` = 2154. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1577 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 87, `updated` = 1493, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch17_20260828_214231.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch17_20260828_214258.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, and zero Cyrillic characters in approved Chinese meanings.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 16 (2026-08-28)

- The sixteenth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p140/b7` through `p148/b3`. Continuation fragments were rejected at `p141/b3`, `p142/b1`, `p143/b1`, `p144/b1`, `p145/b1`, `p145/b7`, `p146/b2`, and `p147/b1`. Merged definitions were trimmed for `мужчина` and `музыка`.
- Important recoveries included `менее`, `меньшинство`, `мера`, `местность`, `местный`, `место`, `месяц`, `метод`, `механизация`, `милиционер`, `миллион`, `министерство`, `минус`, `мировой`, `многие`, `монета`, `молодец`, `мебель`, `медведь`, and `мы`.
- Review sheet status after this batch: `approved` = 1493, `rejected` = 176, `needs_review` = 1, `pending` = 2254. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1490 unique words; `TEM8_RU/TEM8` = 3513. Actual import reported `inserted` = 91, `updated` = 1402, `invalid_approved` = 0, and `skipped` = 0. The dry-run reported one extra insert because it does not simulate same-batch duplicate word handling; the batch contains repeated same-form entries such as `мир` with different senses.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch16_20260828_213648.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch16_20260828_213712.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, and zero Cyrillic characters in approved Chinese meanings.

### Latest checkpoint: TEM4 pending vocabulary LLM review batch 15 (2026-08-28)

- The fifteenth pending batch was reviewed directly with the assistant model. OCR headwords were corrected per word, true `ё` was preserved, and `е`/`ё` changes were made only according to verified Russian spelling.
- The batch covered exactly 100 pending coordinates from `p132/b7` through `p140/b6`. Continuation and merged-fragment handling included rejecting `p135/b12` from `лучше`, `p136/b1` from `любитель`, `p139/b1` from `материя`, and `p140/b1` from `медленный`; `p133/b3` was trimmed to keep only `лиса` and remove the merged `лист` entry.
- Important recoveries included `лететь`, `летний`, `лето`, `летом`, `лечение`, `линейка`, `лицо`, `ловкий`, `ложка`, `луг`, `любимый`, `люди`, `мавзолей`, `майор`, `материя`, `мебель`, `медведь`, `медленный`, and `лестница`. The certain but truncated `критиковать`-style case in this batch was supplemented only when the headword and core meaning were unambiguous.
- Review sheet status after this batch: `approved` = 1401, `rejected` = 168, `needs_review` = 1, `pending` = 2354. The only unresolved manual-review coordinate remains `p154/b2`, intentionally excluded per the user's decision.
- Formal vocabulary counts after import: `TEM4_RU/TEM4` = 1399 unique words; `TEM8_RU/TEM8` = 3513. Import reported `inserted` = 95, `updated` = 1306, `invalid_approved` = 0, and `skipped` = 0.
- Review-sheet backup: `data/processed/words/tem4_words_review_simple_before_pending_batch15_20260828_213113.csv`.
- Database backup: `data/processed/backups/russian_ai_tutor_before_tem4_pending_batch15_20260828_213144.sqlite`.
- Post-apply audit found `missing = [('154', '2')]` only, and zero Cyrillic characters in approved Chinese meanings.
