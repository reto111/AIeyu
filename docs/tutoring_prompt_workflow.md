...
- 语法题需要包含考察点、易错点、选项逐项辨析；词义辨析题需要整理选项词义、搭配、用法差异，必要时只补充少量可靠同根词/派生词。
- 文学题需要介绍作品、作者、文学时期、流派或术语背景；选项涉及多个作家时，需要列出各作家的代表作品。
- 国情题需要补充历史、地理、政治制度、文化常识或中俄名称对应关系。
- 输入资料不足时必须说明需要核验，不能编造。
- 不输出“本次表现”，减少安慰式表达，直接进入错因、知识点和复习动作。
- 输出必须是合法 JSON，便于前端把逐题讲解显示到对应错题下方。
- 逐题解析不再输出“记忆与复习提示”和“同类巩固练习”，并控制文本长度；整体 AI 区仍可保留简短巩固练习安排。
...
- `/api/grade` 同步完成批改和 DeepSeek 错题讲解生成。
- 前端显示“正在批改中”等待状态，等批改与讲解都完成后一起渲染。
- 每道错题讲解显示在对应题目下方。
- 薄弱点排序、复习方案、巩固练习和可追问问题显示在 AI 对话区。
- 学生端不再提供“读取讲解”按钮。
