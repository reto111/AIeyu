...
- 提交后自动批改。
- 显示正确率、错题和薄弱知识点。
- 批改完成后，自动为本次测试生成 DeepSeek 深度错题讲解。
...
生成讲解会把本次错题、选项、学生答案、正确答案、来源标签和薄弱点发送到 DeepSeek。当前原型按用户确认规则在批改后自动生成讲解。追问会把该对话线程历史和学生新问题发送到 DeepSeek，前端仍保留追问前确认。
...
- `/api/grade` 可批改并写入数据库。
- `/api/explain` 可根据本次测试生成深度错题讲解并写入 `ai_tutor_threads` / `ai_tutor_messages`。
- DeepSeek 返回内容按结构拆分：逐题讲解显示在对应错题下方；薄弱点排序、复习方案、巩固练习和可追问问题显示在 AI 对话区。
